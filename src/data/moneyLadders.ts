...
// Every ladder starts at 0.1¢. The TOP prize scales with the game length:
// the shorter the game, the less money is at stake.

const MIN_CENTS = 0.1;

// Top prize per game length — shorter games are worth less.
const MAX_BY_LENGTH: Record<number, number> = {
  5: 100,
  10: 250,
  15: 500,
  20: 1000,
...
// Hand-tuned "nice" ladders for the standard game lengths (0.1¢ -> scaled top).
const fixedLadders: Record<number, number[]> = {
  5: [0.1, 1, 10, 50, 100],
  10: [0.1, 0.5, 1, 5, 10, 25, 50, 100, 175, 250],
  15: [0.1, 0.2, 0.5, 1, 2, 5, 10, 25, 50, 75, 125, 200, 300, 400, 500],
  20: [
    0.1, 0.2, 0.3, 0.5, 1,
    2, 3, 5, 10, 20,
    30, 50, 100, 200, 300,
...
// Top prize for a given game length (scaled so shorter = less money).
function maxCentsFor(n: number): number {
  if (MAX_BY_LENGTH[n]) return MAX_BY_LENGTH[n];
  return niceRound(Math.max(100, Math.min(1000, (1000 * n) / 20)));
}

// Returns a prize ladder of exactly `n` amounts in cents, from 0.1¢ up to the
// length-scaled top prize.
export function getLadder(n: number): number[] {
  if (n <= 1) return [maxCentsFor(n)];
  if (fixedLadders[n]) return fixedLadders[n];

  // Fallback: geometric progression rounded to nice values.
  const maxCents = maxCentsFor(n);
  const ratio = Math.pow(maxCents / MIN_CENTS, 1 / (n - 1));
  const arr: number[] = [];
  for (let i = 0; i < n; i++) {
    if (i === 0) arr.push(MIN_CENTS);
    else if (i === n - 1) arr.push(maxCents);
